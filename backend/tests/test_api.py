...
class TestCollections:
    ...
    def test_delete_collection_with_prompts(self, client: TestClient, sample_collection_data, sample_prompt_data):
        ...
        When a collection is deleted, prompts in that collection should be orphaned
        (collection_id set to None) rather than deleted.
        """
        # Create collection
        ...
        # Verify the prompt still exists but is now orphaned (collection_id = None)
        prompts = client.get("/prompts").json()["prompts"]
        assert len(prompts) == 1
        assert prompts[0]["id"] == prompt_id
        assert prompts[0]["collection_id"] is None
        assert prompts[0]["title"] == sample_prompt_data["title"]
